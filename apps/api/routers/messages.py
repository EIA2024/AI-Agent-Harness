...
async def post_message(
    ...
    The runner owns message persistence (one writer per message), so the API
    layer delegates the user-message insert to :meth:`runner.start` and looks
    up the persisted row afterward.
    """
    ...
    async with session_scope() as s:
        ...
        # The runner persists the user message — fetch it for the response.
...
async def list_messages(
    ...
    """Full message history for a session (chronological, paginated)."""
    async with session_scope() as s:
        result = await s.execute(
            select(Session).where(Session.id == session_id, Session.owner_id == user.id)
        )
